...
-- but requires the session to be outside a transaction block.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_txn_date_district_bedroom
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_txn_district_bedroom_date
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_txn_agg_covering
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_txn_psf
  ON transactions(psf);

-- Price range queries (for histogram)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_txn_price
  ON transactions(price);

-- Sale type filter (New Sale vs Resale)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_txn_sale_type
  ON transactions(sale_type);

-- Area sqft for size range queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_txn_area_sqft
  ON transactions(area_sqft);

-- Remaining lease for tenure queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_txn_remaining_lease
...
-- CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_txn_recent
--   ON transactions(transaction_date, district, bedroom_count)
