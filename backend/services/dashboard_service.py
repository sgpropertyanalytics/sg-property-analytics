...
def query_price_by_age_region(
    params: Dict[str, Any],
    session: Optional[Session] = None
) -> List[Dict]:
    """
    Query total price percentiles grouped by age bucket, region, and bedroom type.

    Uses SQL aggregation only (no pandas) for memory safety on 512MB Render.
    Excludes freehold properties (their lease_start_year is land grant, not TOP).
    Excludes new sale transactions (age < 4 years).

    Args:
        params: Parsed filter params from parse_filter_params():
            - date_from: Python date object
            - date_to: Python date object
            - sale_type_db: DB-format sale type (e.g., 'Resale')
            - districts: List of district codes
            - segments_db: List of regions (CCR, RCR, OCR)
            - tenure_db: Tenure string
        session: Optional SQLAlchemy session
    ...
    Returns:
        List of dicts with keys: age_bucket, region, bedroom_group, observation_count, p25, p50, p75
    """
    # Build filter clauses using :param style ONLY
    filters = [OUTLIER_FILTER]
    bind_params = {}
    ...
    WITH age_bucketed AS (
      SELECT
        ...
        bedroom_count,
        district,
        -- Property age: years since lease started
        CASE
          WHEN lease_start_year IS NOT NULL AND lease_start_year > 0
          THEN EXTRACT(YEAR FROM transaction_date) - lease_start_year
          ELSE NULL
      ...
      WHERE {where_clause}
        ...
        AND price > 0
        AND bedroom_count IS NOT NULL
        -- Exclude freehold (their lease_start_year is land grant, not TOP)
        AND tenure NOT ILIKE '%freehold%'
    ...
    with_age_bucket AS (
      SELECT *,
        CASE
          ...
          ELSE NULL  -- Exclude properties < 4 years (new_sale handled separately)
        END AS age_bucket
      FROM age_bucketed
      WHERE property_age IS NOT NULL AND property_age >= 4
    )
    SELECT
      age_bucket,
      region,
      ...
      PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY price) AS p75
    FROM with_age_bucket
    WHERE age_bucket IS NOT NULL
    GROUP BY age_bucket, region, bedroom_group
    ORDER BY
      CASE age_bucket
        WHEN 'recently_top' THEN 1
        WHEN 'young_resale' THEN 2
        WHEN 'resale' THEN 3
        WHEN 'mature_resale' THEN 4
      END,
      CASE region
        WHEN 'CCR' THEN 1
        WHEN 'RCR' THEN 2
