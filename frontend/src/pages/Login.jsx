...
import { LineChart, ArrowLeft, Shield, TrendingUp, BarChart3, ArrowRight } from 'lucide-react';
 ...
 *
 * Gracefully handles when Firebase isn't configured by showing
 * a prominent demo access option.
 */
function Login() {
  ...
  const handleGoogleSignIn = async () => {
    if (!isConfigured) {
      // If not configured, just go to dashboard
        ...
        >
          ...
          <div className="bg-white rounded-2xl border border-[#94B4C1]/30 shadow-xl shadow-[#213448]/5 p-8">
            ...
            {isConfigured ? (
              ...
              <>
                {/* Demo Access Button (when Firebase not configured) */}
                <button
                  onClick={() => navigate('/market-pulse')}
                  className="group w-full flex items-center justify-center gap-3 px-6 py-4 bg-[#213448] text-[#EAE0CF] rounded-xl hover:bg-[#547792] hover:shadow-lg active:scale-[0.98] transition-all"
                >
                ...
                {/* Info message */}
                <div className="mt-6 p-4 bg-[#EAE0CF]/30 border border-[#94B4C1]/30 rounded-xl">
                  <p className="text-sm text-[#547792]">
                    Google Sign-In will be available soon. For now, enjoy full access to the analytics dashboard.
                  </p>
                </div>
              </>
            )}
