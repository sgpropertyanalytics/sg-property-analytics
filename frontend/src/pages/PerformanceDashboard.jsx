...
export function PerformanceDashboard() {
  ...
  return (
    <div className="min-h-dvh bg-slate-900 text-slate-100 p-3 md:p-6">
      {/* Header */}
      <div className="mb-8">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-700">
              <th className="py-3 px-4 text-left text-slate-400">Chart</th>
              <th className="py-3 px-4 text-left text-slate-400">Status</th>
              <th
                className="py-3 px-4 text-right text-slate-400 cursor-pointer hover:text-cyan-400"
      ...
      <div className="mt-8 p-4 bg-slate-800 rounded-lg">
        <h3 className="text-slate-400 font-bold mb-2">Console Commands</h3>
...
function ChartTimingRow({ entry }) {
  ...
  return (
    <tr className={`border-b border-slate-800 ${exceedsBudget ? 'bg-red-900/20' : ''}`}>
      <td className="py-3 px-4">
        <span className={exceedsBudget ? 'text-red-400' : 'text-slate-200'}>
          {chartName}
        </span>
        {isFilterChange && (
      ...
      <td className={`py-3 px-4 text-right font-mono ${exceedsBudget ? 'text-red-400 font-bold' : 'text-slate-200'}`}>
        {formatMs(timeToData)}
      </td>
