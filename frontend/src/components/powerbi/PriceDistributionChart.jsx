  ...
  return (
    <QueryState loading={loading} error={error} onRetry={refetch} empty={!bins || bins.length === 0} skeleton="bar" height={350}>
      ...
      <div className="px-4 py-3 border-b border-[#94B4C1]/30 shrink-0">
        ...
        {/* Stats row - Grid with equal columns for consistent sizing */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2">
          {stats?.median && (
            <div className="bg-[#213448]/5 rounded px-2.5 py-1.5 text-center">
              <span className="text-[10px] text-[#547792] uppercase tracking-wide">Median</span>
              <div className="text-sm font-semibold font-mono tabular-nums text-[#213448]">{formatPrice(stats.median)}</div>
            </div>
          )}
          {stats?.p25 && stats?.p75 && (
            <div className="bg-[#213448]/5 rounded px-2.5 py-1.5 text-center">
              <span className="text-[10px] text-[#547792] uppercase tracking-wide">Q1–Q3</span>
              <div className="text-sm font-semibold font-mono tabular-nums text-[#213448]">
          ...
          {stats?.iqr && (
            <div className="bg-[#213448]/5 rounded px-2.5 py-1.5 text-center">
              ...
              <div className="text-sm font-semibold font-mono tabular-nums text-[#213448]">{formatPrice(stats.iqr)}</div>
            </div>
          )}
          {modeBucket && (
            <div className="bg-[#213448]/5 rounded px-2.5 py-1.5 text-center">
