...
export const UserProfileMenu = React.memo(function UserProfileMenu({ expanded = false, onOpenSettings }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {/* Avatar */}
        <div className="w-8 h-8 rounded-full overflow-hidden flex-shrink-0 ring-2 ring-[#547792]/30 group-hover:ring-[#EAE0CF]/50 transition-all">
          {user.photoURL ? (
            <img
              src={user.photoURL}
        ...
        {expanded && (
          <div className="flex-1 min-w-0 text-left">
            <div className="text-sm font-medium text-[#EAE0CF] truncate">
              {user.displayName || 'User'}
            </div>
            <div className="text-xs text-[#94B4C1] truncate">
              {user.email}
            </div>
          </div>
        )}
        ...
        {!expanded && isPremium && (
          <div className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-[#EAE0CF] rounded-full flex items-center justify-center">
            <svg className="w-2 h-2 text-[#213448]" fill="currentColor" viewBox="0 0 20 20">
        ...
        {!expanded && (
          ...
          ">
            ...
            <div className="text-xs text-[#94B4C1]">{user.email}</div>
            <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1.5 w-3 h-3 bg-[#213448] rotate-45 border-l border-b border-[#94B4C1]/30" />
          </div>
        )}
      </button>
      ...
      {showDropdown && (
        <div className="absolute left-0 bottom-full mb-2 w-56 bg-white rounded-lg shadow-xl border border-[#94B4C1]/30 py-1 z-50">
          ...
          <div className="px-4 py-3 border-b border-[#94B4C1]/20">
            ...
            <div className="text-sm text-[#547792] truncate">
              {user.email}
            </div>
          </div>
          ...
          <button
            ...
            className="w-full px-4 py-2.5 text-left text-sm text-[#213448] hover:bg-[#EAE0CF]/50 flex items-center gap-3"
          >
          ...
          {/* Subscription status */}
          <div className="px-4 py-2 border-t border-[#94B4C1]/20">
            <div className="text-xs text-[#547792] mb-1">Plan</div>
            <div className={`text-sm font-medium ${isPremium ? 'text-[#213448]' : 'text-[#94B4C1]'}`}>
          ...
          <button
            ...
            className="w-full px-4 py-2.5 text-left text-sm text-red-600 hover:bg-red-50 flex items-center gap-3 border-t border-[#94B4C1]/20"
          >
