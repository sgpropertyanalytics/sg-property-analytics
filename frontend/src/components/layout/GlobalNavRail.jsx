...
export const GlobalNavRail = React.memo(function GlobalNavRail({ activePage, onPageChange, collapsed = false }) {
  ...
  return (
    <nav
      className={`bg-[#213448] w-full flex flex-col py-4 flex-shrink-0 h-full overflow-y-auto overflow-x-hidden ${TRANSITION_CLASS} ${collapsed ? 'px-2' : 'px-3'} ${isPending ? 'opacity-90' : ''}`}
